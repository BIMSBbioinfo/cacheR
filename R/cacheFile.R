...
# #' ```
# #'
# #' @param cache_dir Directory where cache files will be stored. If `NULL`,
# #'   `cacheR_default_dir()` is used.
# #' @param backend   Caching backend, `"rds"` or `"qs"`.
# #' @param file_args Optional character vector of argument names that should be
# #'   treated as directory / path arguments whose contents must be monitored.
# #'
...
#     # We use definition = f so argument matching uses f's signature
#     call_matched <- match.call(definition = f, call = call_f, expand.dots = TRUE)
#     args_for_hash <- as.list(call_matched)[-1]

#     # expressions for all arguments (positional + named + ... expanded)
#     arg_exprs <- as.list(call_matched)[-1]
...
#     ## ================================================================
#     # Strategy:
#     #  - Examine *all* argument expressions.
#     #  - If file_args is provided, only *named* arguments whose name is
#     #    in file_args are filtered; unnamed args are still checked.
#     #  - Each character value is treated as a candidate path, per-directory
#     #    counts keyed by normalized directory path.
...
#         # If file_args specified and this arg has a non-empty name which
#         # is *not* in file_args, skip it. Unnamed args are always checked.
#         if (!is.null(file_args) && nzchar(nm) && !(nm %in% file_args)) {
#           return(integer(0L))
#         }

#         # Evaluate in the caller of the cached function
#         val <- tryCatch(
#           eval(expr, envir = parent.frame()),
#           error = function(e) NULL
#         )
#         if (is.null(val) || !is.character(val)) {
#           # non-character values can't be paths
#           return(integer(0L))
#         }
...
#         # normalise + filter to existing directories
...
#         )
#         if (is.null(val) || !is.character(val)) {
#           # skip functions / non-character globals (e.g. file.path)
#           return(NA_integer_)
#         }
...
# # Find literal and symbol path specs in a function body
# # - literals: character constants used as paths
# # - symbols : variable names that are used as paths
# #
# # It looks at list.files / dir / list.dirs (including namespaced),
# # and handles expressions like:
# #   list.files(file.path(base_dir, "subdir", "nested"))
# #   list.files(path = c("a", "b", path_sym))
# #
...
#   # recurse over the expression tree
#   # collect = TRUE means "we're inside a path argument"
#   recurse <- function(node, collect = FALSE) {
#     # If we're in collecting mode, record bare symbols / character literals
#     if (collect && is.symbol(node)) {
#       symbol_dirs <<- c(symbol_dirs, as.character(node))
#       return(invisible())
#     }
#     if (collect && is.character(node)) {
#       literal_dirs <<- c(literal_dirs, as.character(node))
#       return(invisible())
#     }
...
#         # namespaced call, e.g. base::list.files
...
#         # We are entering a path-sensitive function from "outside"
#         args      <- as.list(node)[-1L]
#         arg_names <- names(args)
#         target    <- NULL

#         # get the path argument (named 'path' or first positional)
...
#         # Start collecting inside the path argument
#         if (!is.null(target)) {
#           recurse(target, collect = TRUE)
#         }

#         # Recurse into all other arguments *without* collecting
#         if (length(args)) {
...
#       } else {
#         # Either:
#         #  - this is not a target function, or
#         #  - we're already in collect mode and this is a nested call
#         # In both cases, just propagate the current collect flag.
#         for (j in seq_along(node)) {
#           recurse(node[[j]], collect = collect)
#         }
#       }
...
#   # Be defensive: never let this kill cacheFile()
#   tryCatch(
...
#         pkg <- as.character(node[[2L]])[1L]
#         pkgs <- c(pkgs, pkg)
#       }

#       # push all children of this call onto the stack
#       stack <- c(as.list(node), stack)
#     }
#   }
...
#     # We use 'expr_item' to strictly separate it from any argument definitions
#     expr_item  <- expr_stack[[1L]]
#     expr_stack <- expr_stack[-1L]

#     if (is.call(expr_item)) {
#       head <- expr_item[[1L]]

#       # detect pkg::fun or pkg:::fun
#       if (identical(head, quote(`::`)) || identical(head, quote(`:::`))) {
#         pkg <- as.character(expr_item[[2L]])[1L]
#         pkgs <- c(pkgs, pkg)
#       }

#       # push all children of this call onto the stack
#       expr_stack <- c(as.list(expr_item), expr_stack)
#     }
#   }
...
#       arg_candidates <- intersect(as.character(file_args), arg_names)
#     } else {
#       arg_candidates <- arg_names
#     }

#     file_counts_args <- NULL
#     if (length(arg_candidates)) {
#       fc <- lapply(arg_candidates, function(argname) {
...
.find_package_deps <- function(fun) {
  ...
  while (length(expr_stack)) {
    ...
    if (is.call(node)) {
      ...
      # --- FIX START ---
      # Filter out missing arguments (e.g. in alist(x=))
      ...
      # --- FIX START ---
      # Filter out missing arguments (CRITICAL: prevents crash on function(p) where p has no default)
      valid <- vapply(children, function(x) !identical(x, quote(expr=)), logical(1))
      children <- children[valid]
      # --- FIX END ---
