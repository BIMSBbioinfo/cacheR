...
`cache_tree_files()` returns a sorted vector of every file tracked across all nodes:

```{r}
tracked <- cache_tree_files()
cat(sprintf("Tracked files (%d):\n", length(tracked)))
for (f in tracked) cat(sprintf("  %s\n", f))
```

### Print a summary

`cache_tree_summary()` gives a quick overview of the full graph:
...
`cache_tree_to_json()` exports nodes and edges as portable JSON — easy to
load in Python, JavaScript, or any downstream tool:
...
`cache_tree_to_dot()` exports the graph in DOT format for rendering with
`dot`, `neato`, or any Graphviz-compatible tool:
...
[targets](https://docs.ropensci.org/targets/) pipeline script.
This is useful when you want to move from interactive exploration to a
production pipeline:
...
The generated `_targets.R` can be used directly with `targets::tar_make()`.

## Cleanup

```{r}
unlink(c(cache_dir, cache_dir2, cache_dir3, data_file, save_path,
         json_path, dot_path, targets_path), recursive = TRUE)
```
